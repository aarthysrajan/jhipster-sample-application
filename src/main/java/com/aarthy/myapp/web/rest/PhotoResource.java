...
/**
 * REST controller for managing {@link com.aarthy.myapp.domain.Photo}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Photo> result = photoRepository
            ...
            .map(existingPhoto -> {
                if (photo.getPhotoId() != null) {
                    existingPhoto.setPhotoId(photo.getPhotoId());
                }
                if (photo.getUserId() != null) {
                    existingPhoto.setUserId(photo.getUserId());
                }
                if (photo.getPicRating() != null) {
                    existingPhoto.setPicRating(photo.getPicRating());
                }
                if (photo.getGenre() != null) {
                    existingPhoto.setGenre(photo.getGenre());
                }
                if (photo.getDescription() != null) {
                    existingPhoto.setDescription(photo.getDescription());
                }
                if (photo.getInContest() != null) {
                    existingPhoto.setInContest(photo.getInContest());
                }
                if (photo.getContestId() != null) {
                    existingPhoto.setContestId(photo.getContestId());
                }
